  ...
  return (
    <div className="mx-auto w-full max-w-2xl p-4">
      ...
      <div className="flex justify-center items-end gap-6">
        {[
          { label: "Savvy Meal Plan", value: 80 },
          { label: "Snack Attack", value: 120 },
          { label: "A La Carte", value: 100 },
        ].map((item, index) => (
          <div key={index} className="flex flex-col items-center w-20">
            <p className="mb-1 text-center">{item.value}</p>
            <div
              style={{ height: `${item.value}px` }}
              className="bg-blue-500 w-16 rounded-t-md"
            ></div>
            <p className="mt-2 text-center w-full">{item.label}</p>
          </div>
        ))}
      </div>
